    ...
    WithViewStore(self.store) { viewStore in
      Form {
        Section(header: Text("Profile")) {
          ...
          Toggle("Protect my posts", isOn: viewStore.binding(\.$protectMyPosts))
        }
        Section(header: Text("Communication")) {
          Toggle("Send notifications", isOn: viewStore.binding(\.$sendNotifications))

          if viewStore.sendNotifications {
            Toggle("Mobile", isOn: viewStore.binding(\.$sendMobileNotifications))

            Toggle("Email", isOn: viewStore.binding(\.$sendEmailNotifications))

            Picker("Top posts digest", selection: viewStore.binding(\.$digest)) {
...
//          viewStore.send(.binding(.set(\.$isLoading, true)))
      ...
      .alert(item: viewStore.binding(\.$alert)) { alert in
